...
@Component({
  ...
  standalone: true,
  imports: [MatCardModule, LoadingSpinnerComponent, CurrencyBrPipe],
  template: `
    ...
    @else {
      <div class="supplier-grid">
        @for (s of data()?.suppliers; track s.id) {
          <mat-card>
            <mat-card-header><mat-card-title>{{ s.name }}</mat-card-title></mat-card-header>
            <mat-card-content>
              <div class="stat"><span class="num">{{ s.totalProducts }}</span> produtos</div>
